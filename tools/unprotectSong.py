...
class unProtectedSong(object):
    ...
    def save_unsecured_song_to_wave(self, file_location, song_verify_hash):
        ...
        while remaining > ENC_CHUNK_SZ:
            ...
            if(chunk_ct < PREVIEW_CT):
                ...
                # other
                chunk = self.unprotect_chunk__encrypt(
                    data[current:(current+ENC_CHUNK_SZ)], self.song_hash, aes_obj)
    ...
    def unprotect_chunk__sample(self, song_chunk, song_hash, public_key):
        ...
        hash_signature = int.from_bytes(
            hash_signature, byteorder='big', signed=False)

        # hash = int.from_bytes(hash_signature, byteorder='big')
        hash_signature_out = pow(
            hash_signature, public_key[1], public_key[0])  # 1 is e 0 is n
    ...
    def unprotect_chunk__encrypt(self, song_chunk, song_hash, aes_obj):
        IV = binascii.unhexlify('000102030405060708090a0b0c0d0e0f')
        aes_obj = AES.new(self.aes_key, AES.MODE_CBC, IV)
...
def parse_header(metadata, public_key):
    ...
    computed_header_hash = compute_hash(
        metadata[0:len(metadata)-HASH_SZ-RSA_KEY_SZ])

    hash_int = int.from_bytes(computed_header_hash,
                              byteorder='big', signed=False)
    signature = int.from_bytes(sig, byteorder='big', signed=False)

    # hash = int.from_bytes(hash_signature, byteorder='big')
    hash_signature_out = pow(
        signature, public_key[1], public_key[0])  # 1 is e 0 is n
...
def main():
    ...
    ownerID, song_size, enabled_region_block, song_hash, song_verify_hash, header_hash, sig = parse_header(
        metadata, public_key)
    ret, common_key = parse_enabled_regions(
        enabled_region_block, args.region_secrets_path)
    ...
    protected_song = unProtectedSong(
        args.infile, metadata, song_hash, public_key, owner_key, common_key, song_size)
